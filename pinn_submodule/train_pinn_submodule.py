...
curdir = os.getcwd()
os.chdir('..')
from data.data_generation import generate_dataset, generate_pipe_flow_dataset, generate_stenosis
...
#     if gen_channel:
#         flows, segmentation_maps, velocity_scales, sdfs, velocity_scale_for_norm_channel = generate_dataset(
#             nsamples=nsamples, 
#             nvox=nvox, 
#             xmin=0, 
#             xmax=10, 
...
#     if gen_pipe:
#         flows_pipe, segmentation_maps_pipe, velocity_scales_pipe, sdfs_pipe, velocity_scale_for_norm_pipe = generate_pipe_flow_dataset(
#             nsamples=nsamples,
#             nvox=nvox, 
#             xmin=0, 
#             xmax=10, 
...
#         velocity_scales = np.concatenate((velocity_scales, velocity_scales_pipe), axis=0)
#         del velocity_scales_pipe
...
#         sdfs = np.concatenate((sdfs, sdfs_pipe), axis=0)
#         del sdfs_pipe
...
#         velocity_scales = velocity_scales_pipe 
#         sdfs = sdfs_pipe 
#         velocity_scale_for_norm = velocity_scale_for_norm_pipe 
    
    # generate stenosis
    flows, segmentation_maps, velocity_scales, sdfs, velocity_scale_for_norm = generate_stenosis(nvox=64) 
    ...
    pde_loss_function = PDELoss(
        ...
        gy=config["gy"],
        gz=config["gz"],
        space_scale=10, # added for new space scale
    )
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (flow_sample, map_sample, vel_scale_sample, sdf_sample) in enumerate(train_dataloader):
            ...
            if use_recon:
                ...
                # if (torch.sum(recon_loss)/torch.sum(weights[:, 1]==1.0)).item() < 5e-3:
                #     mult_weights = weights*1
                #     mult_weights[flow_points, :] = 0
                #     recon_loss *= mult_weights[:, 1]
                #/(vel_scale_repeated**2) # divide to make sure they all have the same weight
                recon_loss = torch.sum(recon_loss)/torch.sum(weights[:, 1]==1.0)
            else:
                recon_loss = torch.tensor(0, dtype=dtype, device=device) 

            # exact reconstruction loss 
            use_exact = False
            if use_exact:
                exact_recon_loss = weights[:, 2]*exact_recon_loss_function.compute_loss(pts_flows_rand, outputs_linear)
                exact_recon_loss = torch.sum(exact_recon_loss)/torch.sum(weights[:, 2]==1.0)
            else:
                exact_recon_loss = torch.tensor(0, dtype=dtype, device=device) 

            # exact reconstruction loss 
            use_vel_norm = False
            if use_vel_norm:
                exact_vel_norm_loss = vel_norm_function.compute_loss(pts_flows_rand, outputs_linear, seg_interpolations_rand, npoints=nbackground + nboundary + nflow)
            else:
                exact_vel_norm_loss = torch.tensor(0, dtype=dtype, device=device)     

            # total losses
            loss = recon_loss + exact_recon_loss + pde_multiplier*pde_loss
            ...
            epoch_exact_loss += exact_recon_loss.item()
            epoch_pde_loss += pde_loss.item()
            epoch_vel_norm_loss += exact_vel_norm_loss.item()
            # specific pde
            if use_pde:
        ...
        if not((epoch+1)%20):
            print(torch.std(outputs_linear[:,0]).item(), torch.std(outputs_linear[:, 1]).item(), torch.std(outputs_linear[:,2]).item(), torch.std(outputs_linear[:,3]).item())
            if config["scheduler"]:
        ...
        writer.add_scalar("Exact Loss/train", epoch_exact_loss/(i+1), epoch)
        writer.add_scalar("Vel Norm Loss/train", epoch_vel_norm_loss/(i+1), epoch)
        ...
        print(
            ...
            f"Exact: {epoch_exact_loss/(i+1):.3e}, ",
            f"Vel Norm:{epoch_vel_norm_loss/(i+1):.3e}"
        )
